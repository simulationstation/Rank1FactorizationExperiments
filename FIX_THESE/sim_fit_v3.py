...
def compute_deviance(y_obs: np.ndarray, y_pred: np.ndarray, data_type: str) -> float:
    """
    FIX B: Compute Poisson deviance with proper convention.
    D = 2 * sum[ y*log(y/mu) - (y - mu) ]
    Convention: y*log(y/mu) = 0 when y = 0
    """
    if data_type == 'poisson':
        deviance = 0.0
        for y_i, mu_i in zip(y_obs, y_pred):
            ...
            if y_i > 0:
                ...
                deviance += 2 * mu_safe  # When y=0: -(-mu) = mu
        return deviance
    else:
        return 0.0
...
def generate_bootstrap_data_poisson(ch_data: Dict, y_pred: np.ndarray,
                                    rng: np.random.Generator) -> Dict:
    """FIX B: Generate bootstrap data for Poisson channel with tiny epsilon."""
    y_pred_safe = np.maximum(y_pred, 1e-9)  # Tiny floor, not 0.1

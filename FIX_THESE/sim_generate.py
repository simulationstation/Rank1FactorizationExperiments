...
def intensity_shape(x: np.ndarray, R: complex, bw_params: BWParams) -> np.ndarray:
    """
    Compute normalized signal shape (matches fitter convention).
    Returns signal / max(signal).
    """
...
def generate_channel_data(x: np.ndarray, R: complex, bw_params: BWParams,
    ...
    # FIX A: Match fitter model exactly
    # Fitter uses: y = scale * signal_norm + bg
    # where signal_norm = signal / max(signal), bg = b0 + b1*(x-mean)

    # Compute normalized signal shape (same as fitter)
    signal_norm = intensity_shape(x, R, bw_params)

    # Background parameters (same convention as fitter)
    scale = mean_scale
    b0 = bg_level * scale * 0.5
    b1 = bg_level * scale * 0.02  # Gentle slope

    # Compute y_true using EXACT same formula as fitter
    bg = b0 + b1 * (x - np.mean(x))
    intensity = scale * signal_norm + np.maximum(bg, 0)
    intensity = np.maximum(intensity, 1e-9)  # Tiny floor for safety
    ...
    if data_type == 'poisson':
        ...
        # FIX B: Compute deviance with proper convention (0 when y=0)
        # D = 2 * sum[ y*log(y/mu) - (y - mu) ] with y*log(y/mu)=0 when y=0
        deviance = 0.0
        for y_i, mu_i in zip(y_obs, y_true):
            mu_safe = max(mu_i, 1e-9)
            if y_i > 0:
                deviance += 2 * (y_i * np.log(y_i / mu_safe) - (y_i - mu_safe))
            else:
                deviance += 2 * mu_safe  # -(-mu) = mu when y=0

...
CALIBRATION FIX version:
- Added diagnostic logging (Λ_obs, p_boot, p_wilks, dof_diff, restarts)
- Added assertions for dof_diff = 2
- Bootstrap uses same restarts as original fit (symmetry fix)
- Λ >= 0 enforcement with assertion
"""
...
DIAGNOSTIC_MODE = False  # Set to True to enable detailed CSV logging
DIAGNOSTIC_DIR = None
DIAGNOSTIC_TEST = None
DIAGNOSTIC_TRIAL = None
DIAGNOSTIC_LOG = []  # Accumulate log entries

def set_diagnostic_mode(enabled: bool, diag_dir: str = None, test_name: str = None, trial: int = None):
    """Enable/disable diagnostic logging mode."""
    global DIAGNOSTIC_MODE, DIAGNOSTIC_DIR, DIAGNOSTIC_TEST, DIAGNOSTIC_TRIAL, DIAGNOSTIC_LOG
    DIAGNOSTIC_MODE = enabled
    DIAGNOSTIC_DIR = diag_dir
    DIAGNOSTIC_TEST = test_name
    DIAGNOSTIC_TRIAL = trial
    DIAGNOSTIC_LOG = []

def log_diagnostic(key: str, value):
    """Log a diagnostic value."""
    global DIAGNOSTIC_LOG
    if DIAGNOSTIC_MODE:
        DIAGNOSTIC_LOG.append((key, value))

def get_diagnostic_log():
    """Return and clear the diagnostic log."""
    global DIAGNOSTIC_LOG
    log = DIAGNOSTIC_LOG.copy()
    DIAGNOSTIC_LOG = []
    return log
...
def bootstrap_pvalue(dataset: Dict, observed_lambda: float,
                     ...
                     n_restarts: int = DEFAULT_STARTS) -> Tuple[float, np.ndarray]:
    """
    ...
    CALIBRATION FIX: Uses same n_restarts for bootstrap as original fit (symmetry).
    """
    from sim_generate import generate_channel_data

    # Log bootstrap configuration
    log_diagnostic('bootstrap_n', n_bootstrap)
    log_diagnostic('bootstrap_restarts', n_restarts)
    ...
    bootstrap_lambdas = []
    n_exceeded = 0  # Count how many bootstrap Λ exceed observed
    ...
    for b in range(n_bootstrap):
        ...
        # CALIBRATION FIX: Use same restarts as original fit (symmetry)
        # Reduced restarts for speed, but same for both constrained/unconstrained
        boot_restarts = max(20, n_restarts // 4)  # At least 20, proportional to main
        fit_unc_b = fit_joint_unconstrained(boot_dataset, n_restarts=boot_restarts)
        fit_con_b = fit_joint_constrained(boot_dataset, n_restarts=boot_restarts)
    ...
    # Compute p-value: P(Λ_boot >= Λ_obs) with tail direction check
    p_value = np.mean(bootstrap_lambdas >= observed_lambda)

    # Log diagnostic info
    log_diagnostic('bootstrap_n_valid', len(bootstrap_lambdas))
    log_diagnostic('bootstrap_lambda_mean', float(np.mean(bootstrap_lambdas)))
    log_diagnostic('bootstrap_lambda_std', float(np.std(bootstrap_lambdas)))
    log_diagnostic('bootstrap_lambda_median', float(np.median(bootstrap_lambdas)))
    log_diagnostic('bootstrap_n_exceeded', n_exceeded)
...
def run_full_fit(dataset: Dict,
    ...
    log_diagnostic('n_restarts', n_restarts)
    log_diagnostic('n_bootstrap', n_bootstrap)
    log_diagnostic('dof_diff', DOF_DIFF)

    # ASSERTION: dof_diff MUST be 2 for complex R constraint
    assert DOF_DIFF == 2, f"dof_diff must be 2 for complex R, got {DOF_DIFF}"
    ...
    if not fit_unc['converged'] or not fit_con['converged']:
        log_diagnostic('fit_converged', False)
    ...
    log_diagnostic('fit_converged', True)
    log_diagnostic('nll_unconstrained', fit_unc['nll'])
    log_diagnostic('nll_constrained', fit_con['nll'])

    # Compute Lambda (dof_diff = 2 for complex R)
    Lambda = compute_lambda(fit_con['nll'], fit_unc['nll'])

    # ASSERTION: Lambda must be >= 0 (constrained can't be better than unconstrained)
    assert Lambda >= 0, f"Lambda must be >= 0, got {Lambda}"

    log_diagnostic('Lambda_obs', Lambda)

    # Wilks p-value (asymptotic reference)
    p_wilks = compute_wilks_pvalue(Lambda, DOF_DIFF)
    log_diagnostic('p_wilks', p_wilks)

    # Check fit-health gates
    gates, health_metrics = check_fit_health(fit_unc, fit_con, chi2_min, chi2_max, deviance_max)
    log_diagnostic('gates', gates)
    log_diagnostic('chi2_dof_a', health_metrics['chi2_dof_a'])
    log_diagnostic('chi2_dof_b', health_metrics['chi2_dof_b'])

    # Check identifiability
    identifiable, ident_reason, ident_metrics = check_identifiability(fit_con)
    log_diagnostic('identifiable', identifiable)
    log_diagnostic('r_spread', ident_metrics['r_spread'])
    log_diagnostic('phi_spread', ident_metrics['phi_spread'])

    # Bootstrap p-value (primary inference)
    if gates == 'PASS':
        p_boot, boot_lambdas = bootstrap_pvalue(dataset, Lambda, n_bootstrap=n_bootstrap, n_restarts=n_restarts)
    else:
        p_boot = np.nan
        boot_lambdas = np.array([])

    log_diagnostic('p_boot', p_boot)

    # Determine verdict
    verdict = determine_verdict(p_boot, gates, identifiable)
    log_diagnostic('verdict', verdict)

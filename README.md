...
## PREDICTIONS: Automated Discovery Factory Results

The following predictions are generated by our automated **Rank-1 Discovery Mine** pipeline, which systematically:
1. Searches public databases (HEPData, arXiv, CERN Open Data) for exotic hadron data
2. Extracts numerical tables from publications
3. Runs the rank-1 factorization test across available channels
4. Reports predictions with full statistical rigor

### Methodology

The rank-1 factorization test examines whether exotic hadron doublets share a common production mechanism. If two states (e.g., Pc(4440) and Pc(4457)) are produced via a single underlying amplitude, the complex coupling ratio:

```
R = g(state₂) / g(state₁) = |R| × exp(iφ)
```

must be **identical across all decay channels**. We test this by:
- Fitting a coherent two-Breit-Wigner model to each channel
- Comparing constrained (R_A = R_B) vs unconstrained (R_A ≠ R_B) likelihoods
- Computing Λ = 2×(NLL_con - NLL_unc), which follows χ²(2) under the null
- Running parametric bootstrap (100-800 replicates) for p-value calibration

**Significance**: A NOT_REJECTED verdict (p ≥ 0.05) supports common production, suggesting the states may be part of a single multiplet (molecular, tetraquark, or hybrid).
...
| State | Mass (MeV) | Width (MeV) | J^P (favored) |
|-------|------------|-------------|---------------|
| Pc(4440) | 4440.3 ± 1.3 | 20.6 ± 4.9 | 1/2⁻ |
| Pc(4457) | 4457.3 ± 0.6 | 6.4 ± 2.0 | 3/2⁻ |
...
3. **Heavy quark spin symmetry**: The doublet structure follows HQSS predictions for hidden-charm pentaquarks

#### Caveats

- This is a **projection-based test** using 1D m(J/ψp) spectra
- Full amplitude analysis would provide stronger constraints
- Tables come from the same underlying dataset (correlated)

#### Data Files

```
discoveries/lhcb_pc_4312_extensions/
├── raw/
│   ├── hepdata_ins1728691_Table%201.csv  # Full spectrum
│   ├── hepdata_ins1728691_Table%202.csv  # mKp > 1.9 GeV cut
│   └── hepdata_ins1728691_Table%203.csv  # cosθ-weighted
└── out/
    ├── rank1_result.json                  # Machine-readable results
    └── RANK1_REPORT.md                    # Human-readable report
```

---

*More predictions will be added as the Discovery Mine pipeline completes processing of additional exotic hadron families.*
